...
| `app.js` | 前端逻辑（Supabase 客户端、认证、图表、拖拽布局、CSV 导入） |
...
## 二、首次导入历史数据
1. 用 GitHub Pages 地址打开网站（见下文部署），点「注册」用邮箱+密码建账号。
2. 登录后点「**导入 CSV**」→ 选择本仓库里的 `seed.csv` → 导入。
   - 这会把你现有的 10 个月数据写入你的账号。（之后新增月份用「+ 添加数据」即可。）

## 三、部署到 GitHub Pages
1. 在 GitHub 新建一个**公开**仓库（如 `finance-dashboard`）。
2. 把本目录 5 个文件推上去（保持根目录）：
   ```bash
   git init
   git add .
...
4. 等一两分钟，访问 `https://你的用户名.github.io/finance-dashboard/` 即可。
